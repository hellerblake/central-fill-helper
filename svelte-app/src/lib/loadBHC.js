import { PDFDocument, degrees, rgb, StandardFonts, PDFFont, PDFPage } from 'pdf-lib';
 ...
 * @param {string[]} lots
 * @param {import('./stores/rack.store').Rack[]} rackData
 * */
export async function createBHCPDF(url, rackData, lots) {
	pdfDoc = await PDFDocument.load(await getPdfArrayBuffer(url));
	font = await pdfDoc.embedFont(StandardFonts.Courier);
	page = pdfDoc.getPages()[0];
	debugger;
	fillsByRack = rackData.map((rack) => {
		const { ['expand']: obj, ...newRack } = rack;
		newRack.sizes = obj?.sizes;
		return newRack;
	});
	lots.forEach((lot) => drawBHCFills(lot));
